...
test('sitemap lists both locales of public pages with hreflang', () => {
  ...
  expect(xml).toContain(`<loc>${origin}/custom-sup-manufacturing</loc>`)
  expect(xml).toContain(`<loc>${origin}/zh/custom-sup-manufacturing</loc>`)
  ...
  expect(xml).toContain(`<loc>${origin}/sup-for-resorts</loc>`)
  expect(xml).toContain(`<loc>${origin}/zh/sup-for-resorts</loc>`)
})

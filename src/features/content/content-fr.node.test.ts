...
test('every en content file has a .fr counterpart (except locale-agnostic site/pages.yaml)', () => {
  ...
  for (const dir of dirs) {
    for (const name of readdirSync(resolve(contentRoot, dir))) {
      if (name.includes('.fr.') || name.includes('.es.') || name.includes('.de.') || name.includes('.it.') || name.includes('.pt.') || name.includes('.nl.') || name.includes('.sv.') || name.includes('.no.') || name.includes('.pl.')) continue
...
test('fr product FAQ pool is French (not Spanish/en fallback)', () => {
  expect(PRODUCT_FAQ_POOL.fr.length).toBe(PRODUCT_FAQ_POOL.en.length)
  const failures: string[] = []
  for (let i = 0; i < PRODUCT_FAQ_POOL.en.length; i++) {
    const qa = `${PRODUCT_FAQ_POOL.fr[i].q} ${PRODUCT_FAQ_POOL.fr[i].a}`
    assertFrench(failures, `faq #${i}`, qa)
    expect(qa).not.toMatch(/[¿¡ñáóú]/i)

...
const handler = async () => {
  ...
  const livePages = loader
    ...
    .map((p: { path: string; content: { seo?: { dateModified?: string } }; meta?: { dateModified?: string } }) => {
      ...
      return { loc: p.path, lastmod: p.meta?.dateModified ?? seo?.dateModified ?? '2026-06-01', es: esPaths.has(p.path), fr: frPaths.has(p.path), it: itPaths.has(p.path), pt: ptPaths.has(p.path), nl: nlPaths.has(p.path), sv: svPaths.has(p.path) }
    })
  const staticPages = [
    ...GUIDES.map((g: { slug: string }) => ({ loc: `/guides/${g.slug}`, lastmod: '2026-06-01', es: true, fr: true, it: true, pt: true, nl: true, sv: true })),
    ...
    ...seriesPages.en.map((s: { slug: string }) => ({ loc: `/products/${s.slug}`, lastmod: '2026-08-15', es: true, fr: true, it: true, pt: true, nl: true, sv: true })),
    { loc: '/evidence/case-studies', lastmod: '2026-06-01', es: true, fr: true, it: true, pt: true, nl: true, sv: true },
    { loc: '/faq', lastmod: '2026-06-01', es: true, fr: true, it: true, pt: true, nl: true, sv: true },
    { loc: '/terms', lastmod: '2026-08-15', es: true, fr: true, it: true, pt: true, nl: true, sv: true },
    { loc: '/privacy', lastmod: '2026-08-15', es: true, fr: true, it: true, pt: true, nl: true, sv: true },
  ]

...
import { getAfarerProducts, getTechArticles, getCaseUses, getEsContentPaths } from '@/features/content/loader'
...
const handler = () => {
  ...
  const paths = [
    ...getAfarerProducts().map((p) => ({ loc: `/products/${p.slug}`, lastmod: '2026-06-01', es: es.has(`/products/${p.slug}`) })),
